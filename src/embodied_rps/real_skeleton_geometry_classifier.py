"""Few-shot classifier over canonical hand-skeleton geometry features."""

from __future__ import annotations

import json
from collections import Counter
from collections.abc import Mapping, Sequence
from dataclasses import dataclass
from pathlib import Path
from typing import Any, Literal

import numpy as np
from numpy.typing import NDArray

from embodied_rps.real_skeleton_failure_features import FINGER_CHAINS, FINGER_NAMES, load_review_clip

GeometryLabel = Literal["rock", "paper", "scissors"]


@dataclass(frozen=True)
class GeometryClassifierConfig:
    """Configuration for the geometry centroid classifier."""

    observation_progress: float = 0.5
    confidence_temperature: float = 0.5

    def __post_init__(self) -> None:
        if not 0.0 < self.observation_progress <= 1.0:
            raise ValueError("observation_progress must be in (0, 1]")
        if self.confidence_temperature <= 0.0:
            raise ValueError("confidence_temperature must be positive")


@dataclass(frozen=True)
class GeometryReviewClip:
    """One loaded skeleton-review clip with canonical landmarks."""

    clip_id: str
    label: GeometryLabel
    source_path: str
    canonical_landmarks: NDArray[np.float32]


@dataclass(frozen=True)
class GeometryClassifier:
    """Nearest-centroid classifier over early skeleton geometry features."""

    labels: tuple[GeometryLabel, ...]
    feature_names: tuple[str, ...]
    feature_mean: NDArray[np.float64]
    feature_scale: NDArray[np.float64]
    centroids: Mapping[GeometryLabel, NDArray[np.float64]]
    config: GeometryClassifierConfig
    train_clip_count: int

    def predict(self, canonical_landmarks: NDArray[np.float32]) -> tuple[GeometryLabel, dict[str, object]]:
        """Predict a gesture from a canonical landmark sequence."""

        feature_map = extract_geometry_feature_map(
            canonical_landmarks,
            observation_progress=self.config.observation_progress,
        )
        feature_array = _feature_array(feature_map, self.feature_names)
        normalized = (feature_array - self.feature_mean) / self.feature_scale
        distances = {
            label: float(np.linalg.norm(normalized - centroid))
            for label, centroid in self.centroids.items()
        }
        scores = np.asarray(
            [-distances[label] / self.config.confidence_temperature for label in self.labels],
            dtype=np.float64,
        )
        scores = scores - float(np.max(scores))
        probabilities_array = np.exp(scores)
        probabilities_array = probabilities_array / float(probabilities_array.sum())
        probabilities = {
            label: float(probability)
            for label, probability in zip(self.labels, probabilities_array.tolist(), strict=True)
        }
        prediction = max(probabilities.items(), key=lambda item: item[1])[0]
        ordered = sorted(probabilities.values(), reverse=True)
        diagnostics: dict[str, object] = {
            "predicted_label": prediction,
            "confidence": float(ordered[0]),
            "confidence_margin": float(ordered[0] - ordered[1]) if len(ordered) > 1 else float(ordered[0]),
            "distances": distances,
            "probabilities": probabilities,
            "features": feature_map,
        }
        return prediction, diagnostics

    def to_json_dict(self) -> dict[str, object]:
        """Return a JSON-serializable classifier summary."""

        return {
            "labels": list(self.labels),
            "feature_names": list(self.feature_names),
            "feature_mean": self.feature_mean.tolist(),
            "feature_scale": self.feature_scale.tolist(),
            "centroids": {label: centroid.tolist() for label, centroid in self.centroids.items()},
            "config": {
                "observation_progress": self.config.observation_progress,
                "confidence_temperature": self.config.confidence_temperature,
            },
            "train_clip_count": self.train_clip_count,
        }


def extract_geometry_feature_map(
    canonical_landmarks: NDArray[np.float32],
    *,
    observation_progress: float,
) -> dict[str, float]:
    """Extract early-only geometry features from canonical landmarks."""

    frames = np.asarray(canonical_landmarks, dtype=np.float32)
    if frames.ndim != 3 or frames.shape[1:] != (21, 3):
        raise ValueError("canonical_landmarks must have shape (T,21,3)")
    if frames.shape[0] == 0:
        raise ValueError("canonical_landmarks must not be empty")
    if not 0.0 < observation_progress <= 1.0:
        raise ValueError("observation_progress must be in (0, 1]")

    frame_count = int(frames.shape[0])
    progress = (np.arange(frame_count, dtype=np.float32) + 1.0) / np.float32(frame_count)
    observed_mask = progress <= np.float32(observation_progress)
    if not bool(np.any(observed_mask)):
        observed_mask[0] = True
    observed = frames[observed_mask]
    extensions = _finger_extensions(observed)
    velocities = _finger_tip_velocities(observed)
    spread = np.linalg.norm(observed[:, 8, :] - observed[:, 20, :], axis=1)
    ring_pinky_curl_gap = (extensions["index"] + extensions["middle"]) * 0.5 - (extensions["ring"] + extensions["pinky"]) * 0.5
    features: dict[str, float] = {
        "observed_frame_count": float(observed.shape[0]),
        "observed_progress": float(progress[np.where(observed_mask)[0][-1]]),
        "spread_mean": float(np.mean(spread)),
        "spread_std": float(np.std(spread)),
        "index_middle_minus_ring_pinky_extension": float(np.mean(ring_pinky_curl_gap)),
        "all_finger_extension_mean": float(np.mean([np.mean(values) for values in extensions.values()])),
        "motion_energy_mean": _motion_energy(observed),
        "jitter_energy_mean": _jitter_energy(observed),
    }
    for finger in FINGER_NAMES:
        values = extensions[finger]
        features[f"early_{finger}_extension_mean"] = float(np.mean(values))
        features[f"early_{finger}_extension_std"] = float(np.std(values))
        features[f"early_{finger}_extension_delta"] = float(values[-1] - values[0]) if len(values) > 1 else 0.0
        finger_velocities = velocities[finger]
        features[f"early_{finger}_velocity_mean"] = float(np.mean(finger_velocities)) if len(finger_velocities) else 0.0
    return {name: float(features[name]) for name in GEOMETRY_FEATURE_NAMES}


def load_review_clips_from_root(root: Path) -> list[GeometryReviewClip]:
    """Load all review JSON clips under a skeleton review artifact root."""

    paths = sorted((root / "landmarks_json").rglob("*.json"))
    if not paths:
        raise ValueError(f"no landmark JSON files found under {root / 'landmarks_json'}")
    clips: list[GeometryReviewClip] = []
    for path in paths:
        payload = json.loads(path.read_text(encoding="utf-8"))
        review = load_review_clip(path)
        label = _infer_geometry_label(payload, path)
        clips.append(
            GeometryReviewClip(
                clip_id=str(payload.get("video_id") or path.stem),
                label=label,
                source_path=review.source_path,
                canonical_landmarks=review.canonical_landmarks,
            )
        )
    return clips


def fit_geometry_classifier(
    clips: Sequence[tuple[str, str, NDArray[np.float32]] | GeometryReviewClip],
    *,
    config: GeometryClassifierConfig,
) -> GeometryClassifier:
    """Fit a nearest-centroid classifier from labeled skeleton clips."""

    normalized_clips = [_normalize_clip(item) for item in clips]
    if not normalized_clips:
        raise ValueError("at least one training clip is required")
    labels = tuple(label for label in ("rock", "paper", "scissors") if any(clip.label == label for clip in normalized_clips))
    if len(labels) < 2:
        raise ValueError("at least two labels are required")
    feature_names = GEOMETRY_FEATURE_NAMES
    matrix = np.asarray(
        [
            _feature_array(
                extract_geometry_feature_map(clip.canonical_landmarks, observation_progress=config.observation_progress),
                feature_names,
            )
            for clip in normalized_clips
        ],
        dtype=np.float64,
    )
    feature_mean = matrix.mean(axis=0)
    feature_scale = matrix.std(axis=0)
    feature_scale[feature_scale < 1e-6] = 1.0
    normalized_matrix = (matrix - feature_mean) / feature_scale
    centroids: dict[GeometryLabel, NDArray[np.float64]] = {}
    for label in labels:
        label_rows = normalized_matrix[[index for index, clip in enumerate(normalized_clips) if clip.label == label]]
        centroids[label] = label_rows.mean(axis=0)
    return GeometryClassifier(
        labels=labels,
        feature_names=feature_names,
        feature_mean=feature_mean,
        feature_scale=feature_scale,
        centroids=centroids,
        config=config,
        train_clip_count=len(normalized_clips),
    )


def evaluate_geometry_classifier(
    classifier: GeometryClassifier,
    clips: Sequence[tuple[str, str, NDArray[np.float32]] | GeometryReviewClip],
) -> tuple[list[dict[str, object]], dict[str, object]]:
    """Evaluate a geometry classifier on labeled clips."""

    normalized_clips = [_normalize_clip(item) for item in clips]
    rows: list[dict[str, object]] = []
    for clip in normalized_clips:
        prediction, diagnostics = classifier.predict(clip.canonical_landmarks)
        passed = prediction == clip.label
        probabilities = diagnostics["probabilities"]
        row = {
            "clip_id": clip.clip_id,
            "source_path": clip.source_path,
            "true_label": clip.label,
            "predicted_label": prediction,
            "passed": passed,
            "confidence": diagnostics["confidence"],
            "confidence_margin": diagnostics["confidence_margin"],
            "rock_probability": _mapping_number(probabilities, "rock"),
            "paper_probability": _mapping_number(probabilities, "paper"),
            "scissors_probability": _mapping_number(probabilities, "scissors"),
            "selected_robot_action": _robot_action_for_prediction(prediction),
            "failure_reason": None if passed else "wrong_prediction",
        }
        rows.append(row)
    label_counts = Counter(str(row["true_label"]) for row in rows)
    passed_counts = Counter(str(row["true_label"]) for row in rows if bool(row["passed"]))
    failed = [row for row in rows if not bool(row["passed"])]
    summary: dict[str, object] = {
        "eval_clip_count": len(rows),
        "passed_clip_count": sum(1 for row in rows if bool(row["passed"])),
        "failed_clip_count": len(failed),
        "accuracy": _safe_rate(sum(1 for row in rows if bool(row["passed"])), len(rows)),
        "per_class": {
            label: {
                "clip_count": int(label_counts.get(label, 0)),
                "passed_count": int(passed_counts.get(label, 0)),
                "accuracy": _safe_rate(int(passed_counts.get(label, 0)), int(label_counts.get(label, 0))),
            }
            for label in ("rock", "paper", "scissors")
        },
        "failed_clip_ids": [str(row["clip_id"]) for row in failed],
        "failure_reason_counts": dict(sorted(Counter(str(row["failure_reason"]) for row in failed).items())),
    }
    return rows, summary


def summarize_geometry_classifier_experiment(
    *,
    train_roots: Sequence[Path],
    eval_root: Path,
    config: GeometryClassifierConfig,
) -> tuple[list[dict[str, object]], dict[str, object], GeometryClassifier]:
    """Train from review roots and evaluate on one review root."""

    train_clips: list[GeometryReviewClip] = []
    train_root_summaries: list[dict[str, object]] = []
    for root in train_roots:
        clips = load_review_clips_from_root(root)
        train_clips.extend(clips)
        train_root_summaries.append(_root_summary(root, clips))
    eval_clips = load_review_clips_from_root(eval_root)
    classifier = fit_geometry_classifier(train_clips, config=config)
    rows, summary = evaluate_geometry_classifier(classifier, eval_clips)
    summary.update(
        {
            "train_clip_count": len(train_clips),
            "train_roots": [root.as_posix() for root in train_roots],
            "eval_root": eval_root.as_posix(),
            "train_root_summaries": train_root_summaries,
            "eval_root_summary": _root_summary(eval_root, eval_clips),
            "classifier": classifier.to_json_dict(),
        }
    )
    return rows, summary, classifier


def _normalize_clip(item: tuple[str, str, NDArray[np.float32]] | GeometryReviewClip) -> GeometryReviewClip:
    if isinstance(item, GeometryReviewClip):
        return item
    clip_id, label, landmarks = item
    return GeometryReviewClip(
        clip_id=clip_id,
        label=_validate_label(label),
        source_path="",
        canonical_landmarks=np.asarray(landmarks, dtype=np.float32),
    )


def _infer_geometry_label(payload: Mapping[str, Any], path: Path) -> GeometryLabel:
    raw_label = str(payload.get("label") or "")
    if raw_label in {"rock", "paper", "scissors"}:
        return _validate_label(raw_label)
    transition_label = str(payload.get("transition_label") or "")
    if transition_label == "rock_to_paper":
        return "paper"
    if transition_label == "rock_to_scissors":
        return "scissors"
    path_text = path.as_posix().lower()
    if "rock_to_paper" in path_text:
        return "paper"
    if "rock_to_scissors" in path_text:
        return "scissors"
    if "/rock/" in path_text or "\\rock\\" in str(path).lower():
        return "rock"
    if "/paper/" in path_text or "\\paper\\" in str(path).lower():
        return "paper"
    if "/scissors/" in path_text or "\\scissors\\" in str(path).lower():
        return "scissors"
    raise ValueError(f"could not infer geometry label for {path}")


def _validate_label(label: str) -> GeometryLabel:
    if label not in {"rock", "paper", "scissors"}:
        raise ValueError(f"unsupported geometry label: {label}")
    return label  # type: ignore[return-value]


def _finger_extensions(frames: NDArray[np.float32]) -> dict[str, NDArray[np.float64]]:
    denominator = np.maximum(
        np.linalg.norm(frames[:, 9, :] - frames[:, 0, :], axis=1).astype(np.float64),
        0.75,
    )
    values: dict[str, NDArray[np.float64]] = {}
    for finger, chain in FINGER_CHAINS.items():
        mcp = chain[0]
        tip = chain[-1]
        values[finger] = np.linalg.norm(frames[:, tip, :] - frames[:, mcp, :], axis=1).astype(np.float64) / denominator
    return values


def _finger_tip_velocities(frames: NDArray[np.float32]) -> dict[str, NDArray[np.float64]]:
    if frames.shape[0] <= 1:
        return {finger: np.asarray([], dtype=np.float64) for finger in FINGER_NAMES}
    values: dict[str, NDArray[np.float64]] = {}
    for finger, chain in FINGER_CHAINS.items():
        tip = chain[-1]
        values[finger] = np.linalg.norm(np.diff(frames[:, tip, :], axis=0), axis=1).astype(np.float64)
    return values


def _motion_energy(frames: NDArray[np.float32]) -> float:
    if frames.shape[0] <= 1:
        return 0.0
    return float(np.linalg.norm(np.diff(frames, axis=0), axis=2).mean())


def _jitter_energy(frames: NDArray[np.float32]) -> float:
    if frames.shape[0] <= 2:
        return 0.0
    return float(np.linalg.norm(np.diff(frames, n=2, axis=0), axis=2).mean())


def _feature_array(feature_map: Mapping[str, float], feature_names: Sequence[str]) -> NDArray[np.float64]:
    return np.asarray([float(feature_map[name]) for name in feature_names], dtype=np.float64)


def _root_summary(root: Path, clips: Sequence[GeometryReviewClip]) -> dict[str, object]:
    return {
        "root": root.as_posix(),
        "clip_count": len(clips),
        "label_counts": dict(sorted(Counter(clip.label for clip in clips).items())),
    }


def _mapping_number(mapping: object, key: str) -> float:
    if isinstance(mapping, Mapping):
        value = mapping.get(key)
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            return float(value)
    return 0.0


def _safe_rate(numerator: int, denominator: int) -> float:
    return float(numerator) / float(denominator) if denominator else 0.0


def _robot_action_for_prediction(prediction: str) -> str:
    if prediction == "rock":
        return "paper"
    if prediction == "paper":
        return "scissors"
    if prediction == "scissors":
        return "rock"
    return ""


GEOMETRY_FEATURE_NAMES: tuple[str, ...] = (
    "observed_frame_count",
    "observed_progress",
    "spread_mean",
    "spread_std",
    "index_middle_minus_ring_pinky_extension",
    "all_finger_extension_mean",
    "motion_energy_mean",
    "jitter_energy_mean",
    "early_index_extension_mean",
    "early_index_extension_std",
    "early_index_extension_delta",
    "early_index_velocity_mean",
    "early_middle_extension_mean",
    "early_middle_extension_std",
    "early_middle_extension_delta",
    "early_middle_velocity_mean",
    "early_ring_extension_mean",
    "early_ring_extension_std",
    "early_ring_extension_delta",
    "early_ring_velocity_mean",
    "early_pinky_extension_mean",
    "early_pinky_extension_std",
    "early_pinky_extension_delta",
    "early_pinky_velocity_mean",
)


__all__ = [
    "GEOMETRY_FEATURE_NAMES",
    "GeometryClassifier",
    "GeometryClassifierConfig",
    "GeometryReviewClip",
    "evaluate_geometry_classifier",
    "extract_geometry_feature_map",
    "fit_geometry_classifier",
    "load_review_clips_from_root",
    "summarize_geometry_classifier_experiment",
]
