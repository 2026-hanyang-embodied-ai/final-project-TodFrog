"""Fail-closed strict validation and promotion preflight for v7d."""
...
V7DStrictValidationPreflightStatus = Literal[
    "blocked_remote_training_not_ready",
    "blocked_two_stage_profiles_missing",
    "ready_for_synthetic_metrics",
    "v7d_strict_gates_failed",
    "ready_for_strict_mp4_validation",
    "ready_for_replay_diagnostics",
    "ready_for_fresh_live_retakes",
    "v7d_promotion_candidate",
]


@dataclass(frozen=True)
class V7DStrictValidationPreflightConfig:
    """Inputs for the v7d strict validation and promotion preflight."""

    project_root: Path = field(default_factory=Path.cwd)
    output_root: Path = Path("artifacts/real_skeleton_v7d_strict_validation_preflight_20260618")
    ...
    synthetic_metrics_root: Path = Path("results/real_skeleton_v7d_two_stage_prompt_window_guard_synthetic_metrics")
    ...
    heldout15_validation_root: Path = Path(
        "artifacts/real_mp4_prediction_validation_heldout15_v7d_real_seeded_prompt_window_guard_20260618"
    )
    replay_diagnostics_root: Path = Path("artifacts/real_skeleton_v7d_replay_diagnostics_20260618")
    fresh_live_root: Path = Path("artifacts/real_skeleton_v7d_fresh_live_retakes_20260618")
    event_manifest_path: Path = Path("artifacts/real_skeleton_schunk_events_v7d_prompt_window_guard_20260618/events.jsonl")
    dataset_search_root: Path = Path("D:/dataset")


def write_v7d_strict_validation_preflight(config: V7DStrictValidationPreflightConfig) -> dict[str, object]:
    """Write a non-mutating strict validation/promotion preflight for v7d."""
    ...
    summary: dict[str, object] = {
        ...
        "promotion_decision": {
            "may_promote_v7d": status == "v7d_promotion_candidate",
            "fallback_policy": None if status == "v7d_promotion_candidate" else "keep_v4_live_demo_fallback",
            "promotion_rule": (
                "promote v7d only after synthetic metrics, original20 20/20, heldout15 15/15, "
        ...
        "notes": [
            ...
            "Replay diagnostics and fresh live retakes cannot override failed original20 or heldout15 strict MP4 gates.",
            "GRU remains local smoke/regression only; promotion requires the two-stage TCN profiles.",
        ],
        "config": _config_summary(project_root=project_root, config=config),
    }
    (output_root / "v7d_strict_validation_preflight_summary.json").write_text(
        json.dumps(_json_ready(summary), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    (output_root / "v7d_strict_validation_preflight_summary.md").write_text(
...
def _remote_training_status(*, project_root: Path, config: V7DStrictValidationPreflightConfig) -> dict[str, object]:
    summary_path = _resolve_path(project_root, config.remote_training_preflight_root) / "v7d_remote_training_preflight_summary.json"
    ...
    if status != "ready_for_remote_tcn_training":
...
def _profile_pair_status(*, project_root: Path, config: V7DStrictValidationPreflightConfig) -> dict[str, object]:
    ...
    stage2 = _profile_status(
        _resolve_path(project_root, config.stage2_profile_json),
        project_root=project_root,
...
def _profile_status(path: Path, *, project_root: Path, expected_labels: Sequence[str], stage_name: str) -> dict[str, object]:
...
def _synthetic_metrics_status(*, project_root: Path, config: V7DStrictValidationPreflightConfig) -> dict[str, object]:
...
def _rock_false_trigger_count(*, root: Path, summary: Mapping[str, object]) -> int:
...
def _overall_status(
    *,
    remote_training: Mapping[str, object],
    profiles: Mapping[str, object],
    synthetic: Mapping[str, object],
    original20: Mapping[str, object],
    heldout15: Mapping[str, object],
    replay: Mapping[str, object],
    live: Mapping[str, object],
) -> tuple[V7DStrictValidationPreflightStatus, str | None, str, list[str]]:
    completed: list[str] = []
    if remote_training.get("status") != "ready_for_remote_tcn_training" or _nonempty(remote_training.get("failures")):
        return "blocked_remote_training_not_ready", "remote_training", "complete local smoke and remote training preflight before strict validation", completed
    completed.append("remote_training_preflight")
    if profiles.get("status") != "passed":
        return "blocked_two_stage_profiles_missing", "two_stage_profiles", "copy back valid stage1/stage2 TCN profiles before strict validation", completed
    completed.append("two_stage_profiles")
    if synthetic.get("status") == "missing":
        return "ready_for_synthetic_metrics", "synthetic_metrics", "run synthetic observation-ratio metrics before MP4 validation", completed
    if synthetic.get("status") != "passed":
        return "v7d_strict_gates_failed", "synthetic_metrics", _fallback_next_action(), completed
    ...
    if original20.get("status") != "passed":
        return "v7d_strict_gates_failed", "original20_strict_validation", _fallback_next_action(), completed
    ...
    if heldout15.get("status") != "passed":
        return "v7d_strict_gates_failed", "heldout15_strict_validation", _fallback_next_action(), completed
    ...
    if replay.get("status") != "passed":
        return "v7d_strict_gates_failed", "replay_diagnostics", _fallback_next_action(), completed
    ...
    if live.get("status") != "passed":
        return "v7d_strict_gates_failed", "fresh_live_retakes", _fallback_next_action(), completed
    completed.append("fresh_live_retakes")
    return "v7d_promotion_candidate", None, "manually promote v7d only after reviewing preserved strict-gate diagnostics", completed


def _fallback_next_action() -> str:
    return "keep_v4_fallback_preserve_v7d_diagnostics_and_document_next_targeted_branch"


def _planned_commands(*, project_root: Path, config: V7DStrictValidationPreflightConfig) -> dict[str, list[str]]:
    stage1_profile = _display_path(_resolve_path(project_root, config.stage1_profile_json), base=project_root)
    stage2_profile = _display_path(_resolve_path(project_root, config.stage2_profile_json), base=project_root)
    ...
    return {
        "write_strict_validation_preflight": ["python", "-m", "embodied_rps.tools.write_v7d_strict_validation_preflight"],
        "validate_original20": [
            "python",
            "-m",
...
def _summary_markdown(summary: Mapping[str, object]) -> str:
    lines = [
        "# V7d Strict Validation Preflight",
        "",
        f"- Status: `{summary.get('status')}`",
        f"- Blocking stage: `{summary.get('blocking_stage')}`",
        f"- Next action: `{summary.get('next_action')}`",
        f"- Promotion eligible: `{_mapping(summary.get('promotion_decision')).get('may_promote_v7d')}`",
        "",
        "## Completed Stages",
        "",
    ]
...
def _sanitize_discovery(discovery: Mapping[str, object]) -> dict[str, object]:
    return {
...
def _config_summary(*, project_root: Path, config: V7DStrictValidationPreflightConfig) -> dict[str, object]:
...
__all__ = ["V7DStrictValidationPreflightConfig", "write_v7d_strict_validation_preflight"]
