"""Evaluate a few-shot temporal curl trajectory classifier from review JSONs."""
...
from embodied_rps.real_skeleton_temporal_trajectory_classifier import (
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Evaluate a few-shot skeleton temporal trajectory classifier.")
    ...
    rows, summary, classifier = summarize_temporal_trajectory_experiment(
        train_roots=args.train_review_root,
        eval_root=args.eval_review_root,
        config=config,
    )
    args.output_root.mkdir(parents=True, exist_ok=True)
    (args.output_root / "temporal_trajectory_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    (args.output_root / "temporal_trajectory_model.json").write_text(
        json.dumps(classifier.to_json_dict(), ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    _write_csv(args.output_root / "temporal_trajectory_clip_predictions.csv", rows)
    _write_markdown(args.output_root / "temporal_trajectory_report.md", summary, rows)
    print(
        json.dumps(
            {
                "summary_path": (args.output_root / "temporal_trajectory_summary.json").as_posix(),
...
def _write_markdown(path: Path, summary: dict[str, object], rows: Sequence[dict[str, object]]) -> None:
    lines = [
        "# Skeleton Temporal Trajectory Classifier Report",
        "",
